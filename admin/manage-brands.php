...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_GET['del'])) {
		$id = $_GET['del'];
		// First check if there are any vehicles associated with this brand
		$checkSql = "SELECT * FROM vehicles WHERE vehicles_brand=:id";
		$checkQuery = $dbh->prepare($checkSql);
		$checkQuery->bindParam(':id', $id, PDO::PARAM_STR);
		$checkQuery->execute();
		if ($checkQuery->rowCount() > 0) {
			// Vehicles are still associated with the brand, cannot delete
			$error = "First delete the vehicles associated with this brand.";
		} else {
			// No vehicles associated, safe to delete the brand
